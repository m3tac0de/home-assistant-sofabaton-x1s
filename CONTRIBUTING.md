...
| `remote-card/`                         | The remote card's TypeScript source (`src/`): shared by the Home Assistant card (`www/remote-card.js`), the web remote that sofabaton-x-server serves (`sofabaton-x-server/src/sofabaton_server/ui/remote/remote-web.js`) and the embeddable element (`src/remote-embed.ts`, built with every internal tag prefixed by `scripts/tag-prefix.mjs` into `ui/embed/` and the npm package). `src/backend/` holds the backend port and its two adapters, `src/shims/` the elements and palette the page needs instead of Home Assistant's frontend; `shims/palette.ts` and `shims/mdi-icons.ts` are generated by `scripts/build-remote-web-assets.mjs`. |
| `packages/sofabaton-x-remote/`         | The npm package of the embeddable element: `package.json`, `index.d.ts`, README and licence notices are committed; `dist/` is built by `npm run build:remote-embed` and published by `sofabaton-x-remote-release.yml` on a `sofabaton-x-remote-v*` tag. |
