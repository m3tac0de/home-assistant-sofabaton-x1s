...
|-----------|------|:--------:|-------------|
| `device` | HA Device | ✓ | Your Sofabaton hub. |
| `activity_id` | int (1–255) | ✓ | Activity (or device) id the mapping belongs to. |
| `button_id` | int (1–255) | ✓ | Physical button code (see [Button ID table](#button-id-reference) below). |
| `device_id` | int (1–255) | ✓ | Device id the command belongs to. |
| `command_id` | int (1–255) | ✓ | Command id within that device. |
| `long_press_device_id` | int (1–255) | – | Device id for the long-press command. Required together with `long_press_command_id`. |
