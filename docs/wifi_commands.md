...
- **Activities left with no devices are removed.** Whenever a device is deleted, the hub automatically deletes any Activity that ends up with **zero devices** as a result — so an Activity whose *only* device is the Wifi Device will be removed by a re-sync. Activities that keep at least one other device survive, but they lose the Wifi Device from their startup/shutdown sequences until the re-sync adds it back. Add a second device to a Wifi-only Activity, or back up before syncing, if you want to keep it.
