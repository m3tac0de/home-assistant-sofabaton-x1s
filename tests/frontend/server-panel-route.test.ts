...
test("hub routes: hub, tab and subtab, with the subtab defaulting per tab", () => {
  ...
  assert.deepEqual(parseRoute("#/aabb"), { kind: "hub", hubId: "aabb", tab: "hub", sub: "devices" });
  assert.deepEqual(parseRoute("#/aabb/nonsense/x"), { kind: "hub", hubId: "aabb", tab: "hub", sub: "devices" });
  assert.deepEqual(parseRoute("#/192.168.1.60/hub"), { kind: "hub", hubId: "192.168.1.60", tab: "hub", sub: "devices" });
...
test("tool pages with their subtabs, bare and legacy hashes", () => {
  ...
  assert.deepEqual(parseRoute("#catalog"), { kind: "hub", hubId: null, tab: "hub", sub: "devices" });
  ...
  assert.deepEqual(parseRoute("#/api/hub"), { kind: "hub", hubId: "api", tab: "hub", sub: "devices" });
});
...
test("hashes round-trip, encode the hub id, and compare by value", () => {
  ...
  assert.equal(hashFor(hubRoute("a b/c", "hub")), "#/a%20b%2Fc/hub/devices");
  assert.deepEqual(parseRoute(hashFor(hubRoute("a b/c", "hub"))), hubRoute("a b/c", "hub"));
  assert.equal(sameRoute(hubRoute("x", "hub", "devices"), parseRoute("#/x/hub")!), true);
