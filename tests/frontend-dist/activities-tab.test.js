...
var SofabatonEditDetailView = class extends i3 {
  ...
  _renderLiveHeaderActions() {
    ...
    return T`
      <div class="detail-title-actions live-actions">
        <button class="live-btn" ?disabled=${!this.dirty} @click=${this._requestDiscard}>${S6.discard}</button>
        <button class="live-btn" ?disabled=${!this.dirty} @click=${this._requestReview}>${S6.reviewChanges}</button>
        <button class="live-btn live-btn--primary" ?disabled=${!this.dirty} @click=${this._requestSync}>${S6.sync}</button>
      </div>
    `;
  }
  render() {
  ...
  _renderButtonBindingsSection(kind) {
    if (this.entityId == null || !this.bundle) return A;
    const S6 = TOOLS_CARD_STRINGS.backup;
        ...
        <div class="quick-access-head">
          <div class="quick-access-head-main">
            <div class="quick-access-title">
              ${isActivity ? S6.activityRunningTitle : S6.buttonBindingsTitle}
            </div>
            <div class="quick-access-sub">
              ${isActivity ? S6.activityRunningSub : S6.buttonBindingsDeviceSub}
            </div>
          </div>
  ...
  _renderActivityBindingsView() {
    ...
    return T`
      ...
      { label: this._selectedEditTitle(), onClick: this._closeBindingsView }
                  ...
                  <div class="detail-title">${S6.bindingsViewTitle}</div>
                </div>
                ${this._renderDirtyChip()}
              </div>
            </div>
          ...
          <div class="detail-scroll">
            <div class="quick-access-section">
              <div class="quick-access-head">
                <div class="quick-access-head-main">
                  <div class="quick-access-title">${S6.buttonBindingsTitle}</div>
                  <div class="quick-access-sub">${S6.buttonBindingsActivitySub}</div>
                </div>
  ...
  _renderActivityRolesBlock() {
    ...
    return renderActivityRolesBlock({
      ...
      customize: {
        label: S6.customizeButtonsToggle,
        meta: bindingCount > 0 ? S6.bindingsConfiguredCount(bindingCount) : S6.bindingsNoneConfigured,
        onOpen: () => {
      ...
      <div class="modal-backdrop" @click=${this._closeRoleConfirm}>
        <div class="dialog small" @click=${(event) => event.stopPropagation()}>
          <div class="dialog-header">
            <div class="dialog-title">${S6.roleConfirmTitle}</div>
            <button class="dialog-close" @click=${this._closeRoleConfirm}><ha-icon icon="mdi:close"></ha-icon></button>
          </div>
          <div class="dialog-body">
            <div class="backup-drawer-sub">${S6.roleConfirmBody}</div>
          </div>
          <div class="dialog-footer">
            <div class="dialog-footer-note"></div>
            <div class="dialog-footer-actions">
              <button class="dialog-btn" @click=${this._closeRoleConfirm}>${S6.roleConfirmCancel}</button>
              <button class="dialog-btn dialog-btn-danger" @click=${this._confirmRoleAssign}>${S6.roleConfirmReplace}</button>
            </div>
          </div>
        </div>
      </div>
  ...
  _renderAddFavoriteDialog() {
    ...
    const commandFields = devices.length === 0 ? T`<div class="backup-drawer-sub">${S6.addFavoriteNoDevices}</div>` : T`
          <div class="decoded-field">
            <label class="decoded-field-label" for="sb-add-fav-device">${S6.addFavoriteDevice}</label>
          ...
          <div class="decoded-field">
            <label class="decoded-field-label" for="sb-add-fav-command">${S6.addFavoriteCommand}</label>
            ${commands.length === 0 ? T`<div class="quick-access-empty">${S6.addFavoriteNoCommands}</div>` : T`
          ...
          <div class="decoded-field">
            <label class="decoded-field-label" for="sb-add-fav-name">${S6.addFavoriteName}</label>
            <input
              id="sb-add-fav-name"
              class="decoded-field-input"
              maxlength="20"
    ...
    const actionFields = T`
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-add-action-name">${S6.addShortcutActionName}</label>
        <input
      ...
      this._addShortcutActionName = event.target.value;
        ...
        />
        <div class="decoded-field-helper">${S6.addShortcutActionHelper}</div>
      </div>
    `;
    const macroFields = T`
      ${macros.length ? T`
            <div class="decoded-field">
              <label class="decoded-field-label" for="sb-add-macro-target">${S6.macroTargetLabel}</label>
              <select
      ...
      this._addFavoriteError = "";
              ...
              >
                ...
                `)}
                <option value="__new__" ?selected=${this._addShortcutMacroMode === "new"}>${S6.macroTargetCreateNew}</option>
              </select>
            </div>
          ` : T`<div class="quick-access-empty">${S6.macroTargetNoExisting}</div>`}
      ${this._addShortcutMacroMode === "new" ? actionFields : A}
    `;
    const haFields = T`
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-ha-action-name">${S6.haActionNameLabel}</label>
        <input
      ...
      this._haActionError = "";
        ...
        />
        <div class="decoded-field-helper">${S6.haActionNameHelper}</div>
      </div>
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-ha-action-address">${S6.haActionAddressLabel}</label>
        <input
      ...
      this._haActionError = "";
        ...
        />
        <div class="decoded-field-helper">${S6.haActionAddressHelper}</div>
      </div>
    `;
    return T`
      <div class="modal-backdrop" @click=${this._closeAddFavoriteDialog}>
        <div class="dialog small" @click=${(event) => event.stopPropagation()}>
          <div class="dialog-header">
            <div class="dialog-title">${S6.addShortcutTitle}</div>
            <button class="dialog-close" @click=${this._closeAddFavoriteDialog}><ha-icon icon="mdi:close"></ha-icon></button>
          </div>
          <div class="dialog-body">
            <div class="decoded-field">
              <label class="decoded-field-label" for="sb-add-shortcut-kind">${S6.addShortcutKindLabel}</label>
              <select
      ...
      this._haActionError = "";
              ...
              >
                <option value="command" ?selected=${kind === "command"}>${S6.shortcutKindCommand}</option>
                <option value="action" ?selected=${kind === "action"}>${S6.shortcutKindAction}</option>
                <option value="ha" ?selected=${kind === "ha"}>${S6.shortcutKindHa}</option>
              </select>
            </div>
          ...
          <div class="dialog-footer">
            ...
            <div class="dialog-footer-actions">
              <button class="dialog-btn" @click=${this._closeAddFavoriteDialog}>${S6.addFavoriteCancel}</button>
              <button class="dialog-btn dialog-btn-primary" @click=${this._applyAddShortcut} ?disabled=${!canAdd}>${S6.addFavoriteAdd}</button>
            </div>
          </div>
        </div>
      </div>
  ...
  _commandLabel(deviceId, commandId) {
    const dId = Number(deviceId || 0);
    const cId = Number(commandId || 0);
    if (!this.bundle || dId <= 0 || cId <= 0) return "";
    return deviceCommandItems(this.bundle, dId).find((command) => command.commandId === cId)?.label ?? "";
  }
  _macroName(buttonId) {
    if (!this.bundle || this.entityId == null) return "";
  ...
  _openEditBindingDialog(kind, buttonId) {
    ...
    this._haActionName = this._bindingTargetKind === "ha" ? this._commandLabel(item.deviceId, item.commandId) : "";
    this._haActionAddress = this._prefillHaActionAddress();
    this._haActionError = "";
    ...
    this._bindingLpHaActionName = this._bindingLpTargetKind === "ha" ? this._commandLabel(this._bindingLpDeviceId, this._bindingLpCommandId) : "";
    this._bindingLpHaActionAddress = this._prefillHaActionAddress();
    this._bindingError = "";
    this._bindingDialogOpen = true;
  }
  ...
  _renderMacroTargetFields(params) {
    ...
    return T`
      ${macros.length ? T`
            <div class="decoded-field">
              <label class="decoded-field-label" for=${`${params.idPrefix}-macro-target`}>${S6.macroTargetLabel}</label>
              <select
              ...
              >
                ...
                `)}
                <option value="__new__" ?selected=${params.mode === "new"}>${S6.macroTargetCreateNew}</option>
              </select>
            </div>
          ` : T`<div class="quick-access-empty">${S6.macroTargetNoExisting}</div>`}
      ${params.mode === "new" ? T`
            <div class="decoded-field">
              <label class="decoded-field-label" for=${`${params.idPrefix}-macro-name`}>${S6.addShortcutActionName}</label>
              <input
  ...
  _renderBindingDialog() {
    ...
    const canSave = this._bindingButtonId != null && (scope === "device" ? this._bindingCommandId != null : targetKind === "command" ? this._bindingDeviceId != null && this._bindingCommandId != null : targetKind === "action" ? true : this._haActionName.trim().length > 0 && this._haActionAddress.trim().length > 0);
    const title = isEdit ? S6.bindingDialogEditTitle(buttonName(Number(this._bindingButtonId))) : S6.bindingDialogAddTitle;
    const commandFields = T`
      ${scope === "activity" ? this._renderBindingSelect({
      id: "sb-binding-device",
      label: S6.bindingTargetDevice,
      value: this._bindingDeviceId,
      options: commandDeviceOptions,
      onChange: this._handleBindingDeviceChange,
      emptyText: S6.bindingNoDevices
    }) : A}
      ${this._renderBindingSelect({
      id: "sb-binding-command",
      label: S6.bindingCommand,
      value: this._bindingCommandId,
      options: commandOptions,
      onChange: this._handleBindingCommandChange,
      emptyText: S6.bindingNoCommands
    })}
    `;
    ...
    const haFields = T`
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-ha-name">${S6.haActionNameLabel}</label>
        <input
      ...
      this._bindingError = "";
        ...
        />
        <div class="decoded-field-helper">${S6.haActionBindingNameHelper}</div>
      </div>
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-ha-address">${S6.haActionAddressLabel}</label>
        <input
    ...
    const lpCommandFields = T`
      ...
      label: S6.bindingLongPressDevice,
      ...
      label: S6.bindingLongPressCommand,
    ...
    const lpHaFields = T`
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-lp-ha-name">${S6.haActionNameLabel}</label>
        <input
          ...
          @input=${(event) => {
      this._bindingLpHaActionName = event.target.value;
      this._bindingError = "";
    }}
        />
        <div class="decoded-field-helper">${S6.haActionBindingNameHelper}</div>
      ...
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-lp-ha-address">${S6.haActionAddressLabel}</label>
        <input
          ...
          @input=${(event) => {
      this._bindingLpHaActionAddress = event.target.value;
      this._bindingError = "";
    }}
        />
        <div class="decoded-field-helper">${S6.haActionAddressHelper}</div>
    ...
    return T`
      ...
      emptyText: S6.bindingNoButtons
    })}
            ${isActivity ? T`
                  <div class="decoded-field">
                    <label class="decoded-field-label" for="sb-binding-kind">${S6.addShortcutKindLabel}</label>
                    <select
                      id="sb-binding-kind"
                      class="decoded-field-input"
                      @change=${this._handleBindingTargetKindChange}
                    >
                      <option value="command" ?selected=${targetKind === "command"}>${S6.shortcutKindCommand}</option>
                      <option value="action" ?selected=${targetKind === "action"}>${S6.shortcutKindAction}</option>
                      <option value="ha" ?selected=${targetKind === "ha"}>${S6.shortcutKindHa}</option>
                    </select>
                  </div>
                ` : A}
            ...
            ${this._bindingLongPressEnabled ? T`
                  ${isActivity ? T`
                        <div class="decoded-field">
                          <label class="decoded-field-label" for="sb-binding-lp-kind">${S6.addShortcutKindLabel}</label>
                          <select
                            id="sb-binding-lp-kind"
                            class="decoded-field-input"
                            @change=${this._handleBindingLpTargetKindChange}
                          >
                            <option value="command" ?selected=${lpTargetKind === "command"}>${S6.shortcutKindCommand}</option>
                            <option value="action" ?selected=${lpTargetKind === "action"}>${S6.shortcutKindAction}</option>
                            <option value="ha" ?selected=${lpTargetKind === "ha"}>${S6.shortcutKindHa}</option>
                          </select>
                        </div>
                      ` : A}
          ...
          <div class="dialog-footer">
            ...
            <div class="dialog-footer-actions">
              <button class="dialog-btn" @click=${this._closeBindingDialog}>${S6.bindingCancel}</button>
              <button class="dialog-btn dialog-btn-primary" @click=${this._applyBinding} ?disabled=${!canSave}>
                ${isEdit ? S6.bindingSave : S6.bindingAdd}
              </button>
            </div>
          </div>
        </div>
  ...
  _renderPowerSetupSection(scope, entityId) {
    ...
    return T`
      <div class="quick-access-section" data-edit-section="power">
        <div class="quick-access-head">
          <div class="quick-access-head-main">
            <div class="quick-access-title">${S6.powerSetupTitle}</div>
            <div class="quick-access-sub">
              ${isDevice ? S6.powerSetupDeviceSub : S6.powerSetupActivitySub}
            </div>
          </div>
        </div>
        ${isDevice ? this._renderPowerControlDropdown(entityId, mode) : A}
        <div class="quick-access-list">
          ${sequencesDisabled ? T`<div class="power-sequences-note">${S6.powerSequencesDisabledNote}</div>` : A}
          <div
            class="quick-access-sortable-container power-sequences"
            data-disabled=${sequencesDisabled ? "true" : "false"}
          >
            ${this._renderPowerSetupRow(scope, entityId, 198, S6.powerOnLabel, sequencesDisabled)}
            ${this._renderPowerSetupRow(scope, entityId, 199, S6.powerOffLabel, sequencesDisabled)}
          </div>
        </div>
      </div>
    `;
  ...
  _renderPowerControlDropdown(deviceId, mode) {
    ...
    return T`
      <div class="power-control" data-open=${open ? "true" : "false"}>
        ...
        >
          <span class="selection-main">
            <span class="selection-label">${selected ? selected.label : S6.powerControlUnset}</span>
            <span class="selection-sub">${selected ? selected.sub : S6.powerControlUnsetSub}</span>
          </span>
...
var SofabatonActivitiesTab = class extends i3 {
  constructor() {
    super(...arguments);
    this.hass = null;
    this.hub = null;
  ...
  static {
    this.styles = [operationProgressStyles, i`
    :host {
      display: flex;
      flex: 1;
      min-height: 0;
  ...
  render() {
    if (this.loading) {
      return T`<div class="tab-panel"><div class="state">${S4.loading}</div></div>`;
    }
    if (this.error) {
      return T`<div class="tab-panel"><div class="state error">${this.error}</div></div>`;
    }
    if (!this.hub || !this.hass) {
      return T`<div class="tab-panel"><div class="state">${S4.selectHub}</div></div>`;
    }
                    ...
                    <ul class="review-entry-list">
                      ${group.entries.map((entry) => T`
                        <li class="review-entry">
                          ${entry.text}
                          ${entry.global ? T`<span class="review-global-note">(${S6.reviewAppliesEverywhere})</span>` : A}
                        </li>
                      `)}
                    </ul>
                  </div>
                `) : T`<div class="review-empty">${S6.reviewEmpty}</div>`}
          </div>
          <div class="dialog-footer">
            <button class="btn btn-danger" @click=${this._openDiscardConfirm}>${S6.reviewDiscardAll}</button>
            <div class="dialog-footer-actions">
              <button class="btn" @click=${this._closeReview}>${S6.reviewKeepEditing}</button>
              <button class="btn btn-primary" @click=${this._requestSync}>${S6.reviewSyncNow}</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="modal-backdrop" @click=${this._closeDiscardConfirm}>
        <div class="dialog dialog--small" @click=${(event) => event.stopPropagation()}>
          ...
          <div class="dialog-footer">
            <span></span>
            <div class="dialog-footer-actions">
              <button class="btn" @click=${this._closeDiscardConfirm}>${S6.discardConfirmCancel}</button>
              <button class="btn btn-danger" @click=${this._discardChanges}>${S6.discardConfirmConfirm}</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="modal-backdrop" @click=${this._closeExitConfirm}>
        <div class="dialog dialog--small" @click=${(event) => event.stopPropagation()}>
          ...
          <div class="dialog-footer">
            ...
            <div class="dialog-footer-actions">
              <button class="btn" @click=${this._closeExitConfirm}>${S6.discardConfirmCancel}</button>
              <button class="btn btn-primary" @click=${this._syncAndLeave}>${S6.exitSyncNow}</button>
            </div>
          </div>
        </div>
      </div>
...
// tests/frontend/activities-tab.test.ts
var ActivitiesTabElement = customElements.get("sofabaton-activities-tab");
var S5 = TOOLS_CARD_STRINGS.activities;
...
test("activities tab does not restore a previous edit session on entry", () => {
  globalThis.window = {
    localStorage: {
      getItem: () => {
        throw new Error("activity edit should not read persisted sessions");
      },
      setItem: () => {
        throw new Error("activity edit should not write persisted sessions");
      },
      removeItem: () => {
        throw new Error("activity edit should not clear persisted sessions");
      }
    }
  };
  try {
    const reader = new ActivitiesTabElement();
