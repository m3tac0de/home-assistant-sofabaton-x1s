...
function commandSlotDefault(idx) {
  return {
    name: `Command ${idx + 1}`,
...
function normalizeCommandsForStorage(nextCommands, sanitizeCommandName) {
  return Array.from({ length: 10 }, (_, idx) => {
    const item = nextCommands?.[idx] || {};
    return {
      ...commandSlotDefault(idx),
      name: sanitizeCommandName(item.name ?? `Command ${idx + 1}`),
      add_as_favorite: item?.add_as_favorite === void 0 ? commandSlotDefault(idx).add_as_favorite : Boolean(item.add_as_favorite),
      hard_button: String(item.hard_button ?? ""),
      long_press_enabled: Boolean(item.long_press_enabled) && Boolean(String(item.hard_button ?? "")),
      activities: Array.isArray(item.activities) ? item.activities.map((id) => String(id)).filter((id) => id !== "") : [],
      action: normalizeCommandAction(item.action),
      long_press_action: normalizeCommandAction(item.long_press_action)
    };
  });
}
function editorHardButtonOptions() {
...
function editorAvailableHardButtonOptions(showX2Keys) {
...
function isCommandConfigured(command, defaults, hasCustomAction) {
  const hasCustomName = String(command?.name || "").trim() !== String(defaults.name);
  const hasFavorite = Boolean(command?.add_as_favorite) !== Boolean(defaults.add_as_favorite);
  ...
  const hasCustomLongPressAction = Boolean(command?.long_press_enabled) && hasCustomAction(command?.long_press_action);
  return hasCustomName || hasFavorite || hasHardButton || Boolean(command?.long_press_enabled) || hasActivities || hasCustomTriggeredAction || hasCustomLongPressAction;
}
...
var SofabatonRemoteCardEditor = class extends HTMLElement {
  ...
  _normalizeCommandAction(action) {
    return normalizeCommandAction(action);
  }
  _commandSlotDefault(idx) {
    return commandSlotDefault(idx);
  }
  _normalizeCommandsForStorage(nextCommands) {
    return normalizeCommandsForStorage(
      nextCommands,
      (name) => this._sanitizeCommandName(name)
    );
  }
  ...
  _ensureCommandSyncWarningModal() {
    ...
    body.innerHTML = "This sync can run for several minutes. During this process, other interactions with the hub are blocked.<br><br>At the end of deployment, the physical remote will be force-resynced. It is recommended to finish your full Wifi Commands setup first, then sync once.";
  ...
  _resolveCommandSyncWarning(confirmed) {
    if (!this._commandSyncWarningModal) return;
    const entityId = String(this._config?.entity || "").trim();
    if (entityId && this._commandSyncWarningOptOutInput && this._commandSyncWarningOptOutInput.checked) {
  ...
  _ensureHubVersionModal() {
    ...
    const body = document.createElement("div");
    body.className = "sb-modal__text";
    body.textContent = "We couldn\u2019t automatically detect your hub model. Select the correct version below \u2014 the change takes effect immediately, no restart needed.";
  ...
  _openHubVersionModal() {
    ...
    const current = String(
      this._hass?.states?.[this._config?.entity]?.attributes?.hub_version || "X1"
    ).toUpperCase();
  ...
  _cloneCommandSlot(slot) {
    return {
      ...
      long_press_enabled: Boolean(slot?.long_press_enabled) && Boolean(String(slot?.hard_button ?? "")),
  ...
  _ensureCommandDraft(slotIdx) {
    ...
    if (!this._commandEditorDrafts || typeof this._commandEditorDrafts !== "object") {
      this._commandEditorDrafts = {};
    }
    if (!this._commandEditorDrafts[slotIdx]) {
      const source = this._commandsList()[slotIdx] || this._commandSlotDefault(slotIdx);
  ...
  _commandActionDetails(action) {
    ...
    const ids = Array.isArray(entityIds) ? entityIds.filter((id) => !!id) : entityIds ? [entityIds] : [];
    const suffix = (value) => {
  ...
  _commandActionRefreshKey(action) {
    ...
    const payload = {
      ...
      target_entity_id: normalizeIdValue(
        target?.entity_id || normalized?.entity_id || normalized?.data?.entity_id || normalized?.service_data?.entity_id
      ),
      target_device_id: normalizeIdValue(
        target?.device_id || normalized?.device_id || normalized?.data?.device_id || normalized?.service_data?.device_id
      ),
      target_area_id: normalizeIdValue(
        target?.area_id || normalized?.area_id || normalized?.data?.area_id || normalized?.service_data?.area_id
      ),
  ...
  _buildCommandActionSelector(pressType) {
    ...
    actionSelector.addEventListener("value-changed", (ev) => {
      ...
      this._updateActiveCommandDraft(
        pressType === "long" ? { long_press_action: nextValue } : { action: nextValue }
      );
      this._commandSaveError = "";
      this._hideUiActionTypeSelector(actionSelector);
      if (this._commandActionRefreshKey(previousValue) !== this._commandActionRefreshKey(nextValue)) {
  ...
  _rebuildCommandActionSelector(pressType) {
    const wrap = pressType === "long" ? this._commandActionEditorLongWrap : this._commandActionEditorShortWrap;
    if (!wrap) return;
  ...
  _editorHardButtonOptions() {
    return editorHardButtonOptions();
  }
  _editorAvailableHardButtonOptions() {
    return editorAvailableHardButtonOptions(this._isEditorX2());
  }
  _commandSlotIcon(hardButton) {
    return commandSlotIcon(hardButton);
  }
  _commandSlotIconColor(hardButton) {
    return commandSlotIconColor(hardButton);
  }
  _renderCommandActionSection(targetWrap = null) {
    ...
    if (this._commandActionEditorHelper) {
      this._commandActionEditorHelper.textContent = activeTab === "long" ? "Select Long-Press Action" : "Select Triggered Action";
    }
    ...
    if (this._commandActionEditorLongWrap) {
      this._commandActionEditorLongWrap.style.display = showLongPress && activeTab === "long" ? "" : "none";
    }
    ...
    this._commandActionEditorSelector = activeTab === "long" ? this._commandActionEditorLongSelector : this._commandActionEditorShortSelector;
  }
  _isCommandConfigured(command, idx) {
    return isCommandConfigured(
      command,
      this._commandSlotDefault(idx),
      (action) => this._commandHasCustomAction(action)
    );
  }
  _closeCommandEditor() {
    const idx = this._activeCommandSlot;
    if (Number.isInteger(idx) && this._commandEditorDrafts && this._commandEditorDrafts[idx]) {
      delete this._commandEditorDrafts[idx];
    }
  ...
  _renderCommandsEditor() {
    ...
    const syncTone = remoteUnavailable ? "error" : this._syncStatusTone(syncStatus);
    ...
    if (remoteUnavailable) {
      syncMessage.textContent = "Remote entity unavailable. Is the app connected?";
    } else if (syncRunning) {
    ...
    syncIcon.setAttribute(
      "icon",
      remoteUnavailable || syncStatus === "failed" ? "mdi:alert-circle-outline" : syncRunning ? "mdi:progress-clock" : "mdi:information-outline"
    );
    ...
    commands.forEach((command, idx) => {
      ...
      if (!configured) {
        ...
        name2.className = "sb-command-slot-name";
        name2.textContent = "Make Command";
        textWrap.appendChild(name2);
      ...
      if (mappedLabel) {
        ...
        if (command.long_press_enabled) {
          ...
          longPressIcon.innerHTML = '<ha-icon icon="mdi:timer-sand-full"></ha-icon>';
      ...
      const actionLabel = details.commandSummary === "No Action configured" ? "No Action configured" : details.service;
      actionSummary.textContent = actionLabel === "No Action configured" ? actionLabel : `> ${actionLabel}`;
    ...
    if (!this._commandEditorModal || !this._commandEditorModal.isConnected) {
      ...
      detailsNote.textContent = "Create a Command in this slot. Give it a name and decide which Activities to apply it to. The name will appear on your remote\u2019s display, in the mobile app, and as the Wifi Command's sensor status.";
      ...
      detailsCancelBtn.addEventListener(
        "click",
        () => this._closeCommandEditor()
      );
      const detailsSaveBtn = document.createElement("button");
      detailsSaveBtn.type = "button";
      detailsSaveBtn.className = "sb-command-dialog-btn sb-command-dialog-btn-primary";
    ...
    if (!this._commandActionEditorModal || !this._commandActionEditorModal.isConnected) {
      const modal = document.createElement("div");
      modal.className = "sb-command-modal";
      modal.addEventListener("click", (ev) => {
      ...
      actionNote.textContent = "Run an Action whenever the command is performed. Configuring an Action is optional; you can create your own automations that trigger from the Wifi Commands sensor.";
      ...
      actionCancelBtn.addEventListener(
        "click",
        () => this._closeCommandActionEditor()
      );
      const actionSaveBtn = document.createElement("button");
      actionSaveBtn.type = "button";
      actionSaveBtn.className = "sb-command-dialog-btn sb-command-dialog-btn-primary";
      actionSaveBtn.textContent = "Save";
      actionSaveBtn.addEventListener(
        "click",
        () => this._saveActiveCommandModal()
      );
    ...
    if (Number.isInteger(activeIdx) && activeIdx >= 0 && activeIdx < commands.length) {
      ...
      activities.forEach((activity) => {
        ...
        chip.addEventListener("click", (ev) => {
          ...
          const nextSet = new Set(
            (this._activeCommandDraft()?.activities || []).map(
              (id) => String(id)
            )
          );
      ...
      this._commandEditorHardButtonSelector.value = active.hard_button ? String(active.hard_button) : "__none__";
