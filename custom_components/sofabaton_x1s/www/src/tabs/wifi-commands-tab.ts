...
interface ActivityEventEntry {
  ...
  stop: WifiCommandAction;
}

export function wifiSectionRows(): Array<{ id: WifiSectionId; label: string; icon: string }> {
  return [
...
export function hubEventRows(): Array<{ key: HubEventKey; label: string; icon: string }> {
  return [
...
export function hubEventModalTitle(key: HubEventKey): string {
  const titles: Record<HubEventKey, string> = {
...
class SofabatonWifiCommandsTab extends LitElement {
  ...
  private _wifiEventsBusy = false;
  private _wifiEventsError = "";
  private _wifiEventDeleteConfirm:
    | { event: WifiEvent; refs: { favorites: number; bindings: number; steps: number } | null }
    | null = null;
  ...
  protected render() {
    if (this.loading) return html`<div class="state">${TOOLS_CARD_STRINGS.common.loading}</div>`;
    if (this.error) return html`<div class="state error">${this.error}</div>`;
    if (!this.hub) return html`<div class="state">${TOOLS_CARD_STRINGS.common.noHubsFound}</div>`;
  ...
  private _renderDeviceListView() {
    ...
    return html`
      <div class="list-scroll">
            ...
            ${this._wifiDevices.map((device) => html`
              ...
              >
                ...
                <div class="device-card-actions">
                  <button class="device-delete-btn" title=${TOOLS_CARD_STRINGS.wifiCommands.deleteDeviceAria} ?disabled=${this._hubCommandLocked() || device.device_key === this._deletingDeviceKey} @click=${(event: Event) => this._promptDeleteDevice(device.device_key, event)}>
        ...
        ` : html`<div class="empty-state-card">${TOOLS_CARD_STRINGS.wifiCommands.emptyDevices}</div>`}
        <div class="sticky-footer">
          ${!canAdd ? html`<div class="wifi-max-devices-note">${TOOLS_CARD_STRINGS.wifiCommands.maximumDevices}</div>` : nothing}
        </div>
      </div>
    `;
  }
    ...
    return this._commandsList()
      ...
      .map(({ command, idx }) => ({
        ...
        label: String(command.name || "").trim()
          || TOOLS_CARD_STRINGS.common.commandFallback(idx + 1),
      }));
  }
  ...
  private _applyHubEventActionsResult(result: { actions?: Record<string, unknown>; activity_actions?: Record<string, unknown> } | null | undefined) {
  ...
  private _normalizeActivityEventActions(raw: unknown): Record<string, ActivityEventEntry> {
    const source = raw && typeof raw === "object" ? (raw as Record<string, unknown>) : {};
    const normalized: Record<string, ActivityEventEntry> = {};
    for (const [key, value] of Object.entries(source)) {
      const activityId = Number.parseInt(String(key).trim(), 10);
      if (!Number.isFinite(activityId) || activityId < 0) continue;
      const phases = value && typeof value === "object" ? (value as Record<string, unknown>) : {};
  ...
  private _activityEventEntry(activityId: string): ActivityEventEntry {
  ...
  private async _runWifiEventMutation(run: () => Promise<{ events?: WifiEvent[] } | undefined>) {
    ...
    try {
      ...
      this._wifiEventsError = String(
        (error as Error)?.message || TOOLS_CARD_STRINGS.wifiCommands.unableSaveAction,
      );
    } finally {
  ...
  private _toggleWifiEventLongPress(event: WifiEvent, enabled: boolean) {
    const entityId = String(this._entityId() || "").trim();
    if (!entityId || !this.hass?.callWS) return;
    // Pure store-flag edit — the long record is always deployed; the flag
    // only gates HA-side long-press action execution.
  ...
  private _openWifiEventDeleteConfirm(event: WifiEvent) {
  ...
  private async _scanWifiEventRefs(event: WifiEvent) {
    ...
    let steps = 0;
    try {
      const res = await this.hass.callWS<{ bundle?: { activities?: Array<Record<string, unknown>> } | null }>({
        type: "sofabaton_x1s/cache/structural_bundle",
        entry_id: entryId,
      });
      const matches = (deviceId: unknown, commandId: unknown) =>
        Number(deviceId) === event.device_id
        && (Number(commandId) === event.command_id || Number(commandId) === event.long_press_command_id);
      for (const activity of res?.bundle?.activities ?? []) {
        const record = activity as {
          favorite_slots?: Array<{ device_id?: unknown; command_id?: unknown }>;
          button_bindings?: Array<{
            device_id?: unknown; command_id?: unknown;
            long_press_device_id?: unknown; long_press_command_id?: unknown;
          }>;
          macros?: Array<{ steps?: Array<{ device_id?: unknown; command_id?: unknown }> }>;
        };
        ...
        for (const binding of record.button_bindings ?? []) {
          if (
            matches(binding?.device_id, binding?.command_id)
            || matches(binding?.long_press_device_id, binding?.long_press_command_id)
          ) bindings += 1;
        }
      ...
      // Reference counts are advisory — fall through with zeros.
    }
  ...
  private _confirmWifiEventDelete() {
    ...
    void this._runWifiEventMutation(async () => {
      try {
        ...
        throw new Error(String(
          (error as Error)?.message || TOOLS_CARD_STRINGS.wifiCommands.wifiEventDeleteFailed,
        ));
      }
    });
  }
  ...
  private _openHubEventEditor(target: HubEventEditorTarget) {
  ...
  private _activeHubEventFlash(): HubEventFireEvent | null {
  ...
  private _flashMatchesHubEventRow(flash: HubEventFireEvent | null, key: HubEventKey): boolean {
  ...
  private _hubEventFlashOverlay(active: boolean, flash: HubEventFireEvent | null) {
    if (!active || !flash) return nothing;
  ...
  private _renderWifiEventsGroup(pressFlash: WifiPressEvent | null) {
    ...
    return html`
      <div class="hub-events">
        ...
        ${this._wifiEventsError
          ? html`<div class="empty-hint">${this._wifiEventsError}</div>`
          : nothing}
        ${events.length ? html`
            ...
            ${events.map((event) => html`
              <li class="hub-event-line">
                ...
                <span class="hub-event-text">
                  ...
                  ${renderAction(event, "short")}${event.long_press_enabled ? html`,
                    ...
                    <ha-switch
                      ...
                      @change=${(changeEvent: Event) => {
                        this._toggleWifiEventLongPress(
                          event,
                          Boolean((changeEvent.target as { checked?: boolean }).checked),
                        );
                      }}
                    ></ha-switch>
  ...
  private _renderWifiEventDeleteConfirm() {
    const confirm = this._wifiEventDeleteConfirm;
    if (!confirm) return nothing;
    const W = TOOLS_CARD_STRINGS.wifiCommands;
    const refs = confirm.refs;
    const body = refs == null
      ? W.wifiEventDeleteScanning
      : refs.favorites + refs.bindings + refs.steps === 0
        ? W.wifiEventDeleteNoRefs
        : W.wifiEventDeleteRefs(refs.favorites, refs.bindings, refs.steps);
    return html`
      <div class="modal-backdrop" @click=${() => { this._wifiEventDeleteConfirm = null; }}>
        <div class="dialog small" @click=${(event: Event) => event.stopPropagation()}>
          <div class="dialog-header">
            <div class="dialog-title">${W.wifiEventDeleteConfirmTitle(confirm.event.name)}</div>
            <button class="dialog-close" @click=${() => { this._wifiEventDeleteConfirm = null; }}><ha-icon icon="mdi:close"></ha-icon></button>
          </div>
          <div class="dialog-body">
          ...
          <div class="dialog-footer">
            <div class="dialog-footer-actions">
              <button class="dialog-btn" @click=${() => { this._wifiEventDeleteConfirm = null; }}>${W.createModalCancel}</button>
  ...
  private _renderActivityEventLine(activity: { id: number; name: string }, flash: HubEventFireEvent | null) {
    const idKey = String(activity.id);
    const entry = this._activityEventEntry(idKey);
    const renderPhase = (phase: ActivityEventPhase) => {
  ...
  private _hubEventEditorTitle(target: HubEventEditorTarget): string {
    ...
    return target.phase === "start"
      ? TOOLS_CARD_STRINGS.wifiCommands.activityEventStartModalTitle(name)
      : TOOLS_CARD_STRINGS.wifiCommands.activityEventStopModalTitle(name);
  }
      ...
      <div class="slot-btn">
        <div class="slot-actions">
          ${this._supportsPowerInputConfig() && command.is_power_on && command.is_power_off
            ? html`<span class="slot-flag power-both" title=${TOOLS_CARD_STRINGS.wifiCommands.powerBothCommand}><ha-icon icon="mdi:power"></ha-icon></span>`
            : this._supportsPowerInputConfig() && command.is_power_on
              ? html`<span class="slot-flag power-on" title=${TOOLS_CARD_STRINGS.wifiCommands.powerOnCommand}><ha-icon icon="mdi:power"></ha-icon></span>`
              : this._supportsPowerInputConfig() && command.is_power_off
                ? html`<span class="slot-flag power-off" title=${TOOLS_CARD_STRINGS.wifiCommands.powerOffCommand}><ha-icon icon="mdi:power"></ha-icon></span>`
                : nothing}
        ...
        <button class="slot-main" @click=${() => this._openCommandEditor(idx)}>
          <span class="slot-text-wrap">
            <span class="slot-name">${String(command.name || "").trim()
              || TOOLS_CARD_STRINGS.common.commandFallback(idx + 1)}</span>
  ...
  private _commandHasCustomAction(action: unknown) {
    ...
    return details.service !== "perform-action"
      || details.entities !== TOOLS_CARD_STRINGS.wifiCommands.noTargetEntity;
  }
  ...
  private _inputFlagTitle(command: WifiCommandSlot) {
    ...
    return activityId
      ? TOOLS_CARD_STRINGS.wifiCommands.inputFor(this._activityName(activityId))
      : TOOLS_CARD_STRINGS.wifiCommands.inputCommand;
  }
  ...
  private _inputActivityReplacementLabel() {
    ...
    const name = String(replacement.slot.name || "").trim()
      || TOOLS_CARD_STRINGS.common.commandFallback(replacement.index + 1);
  ...
  private _hardButtonConflictInfo(buttonId: string) {
    ...
    for (let idx = 0; idx < commands.length; idx += 1) {
      ...
      if (String(commands[idx].hard_button || "").trim() === buttonId) {
        const name = String(commands[idx].name || "").trim()
          || TOOLS_CARD_STRINGS.common.commandFallback(idx + 1);
        return {
    ...
    for (const device of this._wifiDevices) {
      ...
      for (let idx = 0; idx < device.commands.length; idx += 1) {
        if (String(device.commands[idx]?.hard_button || "").trim() === buttonId) {
          const name = String(device.commands[idx]?.name || "").trim()
            || TOOLS_CARD_STRINGS.common.commandFallback(idx + 1);
