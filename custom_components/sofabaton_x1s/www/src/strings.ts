...
export const TOOLS_CARD_STRINGS_EN = {
  ...
  backup: {
    ...
    learnFromHubDescription:
      "The hub's receiver listens for one button press from a physical remote pointed at it.",
    ...
    learnHubLearned: (timings: number, khz: string) =>
      `Learned from the remote: ${timings} timings at ${khz} kHz. Test it, then Save.`,
    ...
    learnHubInterrupted: (by: string) =>
      `Other hub traffic ended the learning window (${by}). Keep the paired remote untouched and try again.`,
