...
export const TOOLS_CARD_STRINGS = {
  ...
  backup: {
    ...
    deleteFavoriteTitle: (name: string) => `Delete favorite "${name}"?`,
    ...
    deleteImpactFavorites: (count: number) =>
      `${count} favorite${count === 1 ? "" : "s"} will be removed`,
    deleteImpactMacroSteps: (count: number) =>
      `${count} macro step${count === 1 ? "" : "s"} will be removed`,
    ...
    waitLabel: (seconds: string) => `Wait ${seconds}s`,
    delaySuffix: (seconds: string) => `then wait ${seconds}s`,
