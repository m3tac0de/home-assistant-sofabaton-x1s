...
//
// `long_press` is a card-level block (not per layout): holding one of the
// selected buttons repeats its command, like on the physical remote. The
// feature is off unless `long_press.enabled` is true; once enabled, every
...
import type { LongPressConfig, RemoteCardConfig } from "./remote-card-types";
...
export function longPressBlock(
  config: Pick<RemoteCardConfig, "long_press"> | null | undefined,
): LongPressConfig {
...
/**
 * Next `long_press` block for an enable flip. Enabling stores only
 ...
 */
export function longPressEnabledPatch(enabled: boolean): LongPressConfig | undefined {
  return enabled ? { enabled: true } : undefined;
}

/**
 * Next `long_press` block for a group selection. Selected groups are stored
...
export function longPressGroupsPatch(
  ...
  const next: LongPressConfig = { ...current, enabled: true };
