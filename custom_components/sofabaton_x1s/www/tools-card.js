...
function backupActivityOptions(hub) {
  return hubActivities(hub).map((activity) => ({
    id: Number(activity.id),
    label: String(activity.name || `Activity ${activity.id}`),
    meta: `${Number(activity.favorite_count || 0)} favs \xB7 ${Number(activity.macro_count || 0)} macros`
  }));
}
...
var SofabatonBackupTab = class extends i4 {
      ...
      <div class="accordion-section ${isOpen ? "open" : ""}">
        ...
        ${isOpen ? b2`
          <div class="acc-body">
            ...
            ${this._backupSuccess ? this._renderStatus("success", "mdi:check-circle-outline", this._backupSuccess) : A}
            ${isRunning && this._backupProgress ? this._renderProgressCard(this._backupProgress, "backup") : A}
            <div class="selection-grid">
              <div class="selection-card">
                ...
                <div class="selection-list">
                  ${activities.length ? activities.map((activity) => b2`
                    <label class="selection-row">
                      <input
                        ...
                        @change=${(event) => this._setBackupActivity(activity.id, event.currentTarget.checked)}
                      />
                  ...
                  `) : b2`<div class="selection-empty">No cached activities available yet.</div>`}
                </div>
              </div>
              <div class="selection-card">
                <div class="selection-card-header">
      ...
      return b2`
                      <label class="selection-row ${locked ? "locked" : ""}">
                        <input
                          ...
                          @change=${(event) => this._setBackupDevice(device.id, event.currentTarget.checked)}
                        />
                        <span class="selection-main">
                          <span class="selection-label">${device.label}</span>
                          ${device.meta ? b2`<span class="selection-sub">${locked ? `${device.meta} \xB7 included by full-hub backup` : device.meta}</span>` : A}
                        </span>
                      </label>
                    `;
    }) : b2`<div class="selection-empty">No cached devices available yet.</div>`}
                </div>
              </div>
            </div>
            <div class="action-row">
              <button class="primary-btn" ?disabled=${this._backupActionDisabled()} @click=${this._runBackup}>Start backup</button>
              ${this._backupProgress?.backup ? b2`<button class="secondary-btn" @click=${this._downloadLatestBackup}>Download JSON</button>` : A}
            </div>
          </div>
        ` : A}
      </div>
                    ...
                    ${activityOptions.length ? activityOptions.map((activity) => b2`
                      <label class="selection-row">
                        <input
                          ...
                          @change=${(event) => this._setRestoreActivity(activity.id, event.currentTarget.checked)}
                        />
                        <span class="selection-main">
                          <span class="selection-label">${activity.label}</span>
                          ${activity.meta ? b2`<span class="selection-sub">${activity.meta}</span>` : A}
      ...
      return b2`
                        <label class="selection-row ${forced ? "locked" : ""}">
                          <input
                            ...
                            @change=${(event) => this._setRestoreDevice(device.id, event.currentTarget.checked)}
                          />
                          <span class="selection-main">
                            <span class="selection-label">${device.label}</span>
  ...
  _setBackupActivity(activityId, checked) {
    ...
    if (!this._backupActivityIds.length && this.cacheHub) {
      this._backupDeviceIds = this._backupDeviceIds.length ? [...new Set(this._backupDeviceIds)].sort((left, right) => left - right) : backupDeviceOptions(this.cacheHub).map((device) => device.id);
    }
  }
