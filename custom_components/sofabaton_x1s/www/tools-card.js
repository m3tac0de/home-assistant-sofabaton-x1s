...
function renderSettingsTab(params) {
  ...
  const batteryLevel = remoteBattery?.supported && typeof remoteBattery.level === "number" ? Math.max(0, Math.min(100, Math.round(remoteBattery.level))) : null;
  const batteryUpdated = String(remoteBattery?.last_updated ?? "").trim();
  const batteryStatus = batteryLevel == null ? String(remoteBattery?.attributes?.last_poll_status ?? "Waiting") : batteryUpdated ? "Updated" : "Current";
...
var SofabatonEditDetailView = class extends i4 {
  constructor() {
    ...
    this._handleBindingTargetKindChange = (event) => {
      ...
      this._haActionName || (this._haActionName = this._commandLabel(this._bindingDeviceId, this._bindingCommandId));
      this._haActionAddress || (this._haActionAddress = this._prefillHaActionAddress());
    };
    ...
    this._handleBindingLpTargetKindChange = (event) => {
      ...
      this._bindingLpHaActionName || (this._bindingLpHaActionName = this._commandLabel(this._bindingLpDeviceId, this._bindingLpCommandId));
      this._bindingLpHaActionAddress || (this._bindingLpHaActionAddress = this._prefillHaActionAddress());
    };
  ...
  _renderAddFavoriteDialog() {
    ...
    const haFields = b2`
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-ha-action-name">${S5.haActionNameLabel}</label>
        <input
      ...
      this._haActionError = "";
        ...
        />
        <div class="decoded-field-helper">${S5.haActionNameHelper}</div>
      </div>
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-ha-action-address">${S5.haActionAddressLabel}</label>
        <input
      ...
      this._haActionError = "";
        ...
        />
        <div class="decoded-field-helper">${S5.haActionAddressHelper}</div>
      </div>
    `;
    return b2`
      ...
      this._haActionError = "";
              ...
              >
                ...
                <option value="ha" ?selected=${kind === "ha"}>${S5.shortcutKindHa}</option>
              </select>
            </div>
  ...
  _renderBindingDialog() {
    ...
    const haFields = b2`
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-ha-name">${S5.haActionNameLabel}</label>
        <input
      ...
      this._bindingError = "";
        ...
        />
        <div class="decoded-field-helper">${S5.haActionBindingNameHelper}</div>
      </div>
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-ha-address">${S5.haActionAddressLabel}</label>
        <input
    ...
    const lpHaFields = b2`
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-lp-ha-name">${S5.haActionNameLabel}</label>
        <input
      ...
      <div class="decoded-field">
        <label class="decoded-field-label" for="sb-binding-lp-ha-address">${S5.haActionAddressLabel}</label>
        <input
            ...
            ${isActivity ? b2`
                  <div class="decoded-field">
                    ...
                    >
                      ...
                      <option value="ha" ?selected=${targetKind === "ha"}>${S5.shortcutKindHa}</option>
                    </select>
                  </div>
                ` : A}
                  ...
                  ${isActivity ? b2`
                        <div class="decoded-field">
                          ...
                          >
                            ...
                            <option value="ha" ?selected=${lpTargetKind === "ha"}>${S5.shortcutKindHa}</option>
                          </select>
                        </div>
                      ` : A}
...
function diffDeviceForReview(baseline, edited, deviceId) {
  const D2 = TOOLS_CARD_STRINGS.activities.deviceReview;
  const buckets = {
    power: [],
    buttons: [],
    macros: []
  ...
  if (idleBefore !== idleAfter) {
    ...
    buckets.power.push({ text: D2.powerControlChanged(label) });
  }
  ...
  for (const [buttonId, text] of [
    [POWER_ON_MACRO_BUTTON_ID2, D2.powerOnChanged],
    [POWER_OFF_MACRO_BUTTON_ID2, D2.powerOffChanged]
  ]) {
  ...
  for (const [buttonId, macro] of editMacros) {
    ...
    if (!before) {
      buckets.macros.push({ text: D2.macroAdded(name) });
      continue;
    }
    ...
    if (renamed) buckets.macros.push({ text: D2.macroRenamed(String(before?.name || ""), name) });
    if (stepsChanged) buckets.macros.push({ text: D2.macroChanged(name) });
  }
  for (const [buttonId, macro] of baseMacros) {
    if (isPower(buttonId) || editMacros.has(buttonId)) continue;
    buckets.macros.push({ text: D2.macroRemoved(String(macro?.name || `Macro ${buttonId}`)) });
  }
  ...
  for (const [buttonId, item] of editBindings) {
    ...
    if (changed) buckets.buttons.push({ text: D2.bindingBound(item.buttonName, item.shortPressLabel) });
  }
  for (const [buttonId, item] of baseBindings) {
    if (!editBindings.has(buttonId)) buckets.buttons.push({ text: D2.bindingCleared(item.buttonName) });
  }
...
var SofabatonActivitiesTab = class extends i4 {
  ...
  _reviewSectionTitle(section) {
    ...
    if (this.kind === "device") {
      switch (section) {
        case "power":
          return D2.sectionPower;
        case "buttons":
          return D2.sectionButtons;
        case "macros":
          return D2.sectionMacros;
      }
    }
    switch (section) {
      case "devices":
    ...
    return b2`
      <div class="modal-backdrop" @click=${this._closeDiscardConfirm}>
        <div class="dialog dialog--small" @click=${(event) => event.stopPropagation()}>
          ...
          <div class="dialog-body"><div class="dialog-text">${S5.discardConfirmBody(this.kind)}</div></div>
          <div class="dialog-footer">
            <span></span>
            <div class="dialog-footer-actions">
